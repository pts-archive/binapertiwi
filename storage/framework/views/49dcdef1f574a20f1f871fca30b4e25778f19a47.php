...
<script nonce="<?php echo e(csrf_token()); ?>">
    ...
    function genericRowLinkFormatter(destination) {
        return function (value,row) {
            if (value) {
                return '<a href="<?php echo e(url('/')); ?>/' + destination + '/' + row.id + '"> ' + value + '</a>';
            }
        };
    }
    ...
    function genericColumnObjLinkFormatter(destination) {
        return function (value,row) {
            if ((value) && (value.status_meta)) {
                ...
                return '<nobr><a href="<?php echo e(url('/')); ?>/' + destination + '/' + value.id + '" data-toggle="tooltip" title="'+ status_meta[value.status_meta] + '"> <i class="fa ' + icon_style + ' text-' + text_color + '"></i> ' + value.name + ' ' + text_help + ' </a> </nobr>';
                ...
                return '<nobr><a href="<?php echo e(url('/')); ?>/' + dpolymorphicItemFormatterest + dest + '/' + value.id + '"> ' + value.name + '</a></span>';
            }
        };
    }

    function hardwareAuditFormatter(value, row) {
        return '<a href="<?php echo e(url('/')); ?>/hardware/audit/' + row.id + '/" class="btn btn-sm bg-yellow" data-tooltip="true" title="Audit this item"><?php echo e(trans('general.audit')); ?></a>';
    }
    ...
    function genericActionsFormatter(owner_name, element_name) {
        ...
        return function (value,row) {
            ...
            if ((row.available_actions) && (row.available_actions.clone === true)) {
                actions += '<a href="<?php echo e(url('/')); ?>/' + dest + '/' + row.id + '/clone" class="btn btn-sm btn-info" data-tooltip="true" title="Clone Item"><i class="fa fa-copy" aria-hidden="true"></i><span class="sr-only">Clone</span></a>&nbsp;';
            }

            if ((row.available_actions) && (row.available_actions.update === true)) {
                actions += '<a href="<?php echo e(url('/')); ?>/' + dest + '/' + row.id + '/edit" class="btn btn-sm btn-warning" data-tooltip="true" title="Update Item"><i class="fa fa-pencil" aria-hidden="true"></i><span class="sr-only">Update</span></a>&nbsp;';
            }

            if ((row.available_actions) && (row.available_actions.delete === true)) {
                actions += '<a href="<?php echo e(url('/')); ?>/' + dest + '/' + row.id + '" '
            ...
            if ((row.available_actions) && (row.available_actions.restore === true)) {
                actions += '<a href="<?php echo e(url('/')); ?>/' + dest + '/' + row.id + '/restore" class="btn btn-sm btn-warning" data-toggle="tooltip" title="Restore"><i class="fa fa-retweet"></i></a>&nbsp;';
            }
    ...
    function polymorphicItemFormatter(value) {
        ...
        if ((value) && (value.type)) {
            ...
            return '<nobr><a href="<?php echo e(url('/')); ?>/' + item_destination +'/' + value.id + '" data-tooltip="true" title="' + value.type + '"><i class="fa ' + item_icon + ' text-<?php echo e($snipeSettings->skin!='' ? $snipeSettings->skin : 'blue'); ?> "></i> ' + value.name + '</a></nobr>';
    ...
    function licenseSeatInOutFormatter(value, row) {
        ...
        if ((row.available_actions.checkout == true) && (row.user_can_checkout == true) && ((!row.asset_id) && (!row.assigned_to))) {
            return '<a href="<?php echo e(url('/')); ?>/licenses/' + row.license_id + '/checkout/'+row.id+'" class="btn btn-sm bg-maroon" data-toggle="tooltip" title="Check this item out"><?php echo e(trans('general.checkout')); ?></a>';
        } else {
            return '<a href="<?php echo e(url('/')); ?>/licenses/' + row.id + '/checkin" class="btn btn-sm bg-purple" data-toggle="tooltip" title="Check in this license seat."><?php echo e(trans('general.checkin')); ?></a>';
        }
    ...
    function genericCheckinCheckoutFormatter(destination) {
        return function (value,row) {
            ...
            if ((row.available_actions.checkout == true) && (row.user_can_checkout == true) && ((!row.asset_id) && (!row.assigned_to))) {
                    return '<a href="<?php echo e(url('/')); ?>/' + destination + '/' + row.id + '/checkout" class="btn btn-sm bg-maroon" data-toggle="tooltip" title="Check this item out"><?php echo e(trans('general.checkout')); ?></a>';

            // The user is allowed to check items out, but the item is not deployable
            } else if (((row.user_can_checkout == false)) && (row.available_actions.checkout == true) && (!row.assigned_to)) {
                return '<div  data-toggle="tooltip" title="This item has a status label that is undeployable and cannot be checked out at this time."><a class="btn btn-sm bg-maroon disabled"><?php echo e(trans('general.checkout')); ?></a></div>';

            // The user is allowed to check items in
            } else if (row.available_actions.checkin == true)  {
                if (row.assigned_to) {
                    return '<a href="<?php echo e(url('/')); ?>/' + destination + '/' + row.id + '/checkin" class="btn btn-sm bg-purple" data-toggle="tooltip" title="Check this item in so it is available for re-imaging, re-issue, etc."><?php echo e(trans('general.checkin')); ?></a>';
                } else if (row.assigned_pivot_id) {
                    return '<a href="<?php echo e(url('/')); ?>/' + destination + '/' + row.assigned_pivot_id + '/checkin" class="btn btn-sm bg-purple" data-toggle="tooltip" title="Check this item in so it is available for re-imaging, re-issue, etc."><?php echo e(trans('general.checkin')); ?></a>';
                }
    ...
    function assetRequestActionsFormatter (row, value) {
        if (value.available_actions.cancel == true)  {
            return '<form action="<?php echo e(url('/')); ?>/account/request-asset/'+ value.id + '" method="GET"><button class="btn btn-danger btn-sm" data-toggle="tooltip" title="Cancel this item request"><?php echo e(trans('button.cancel')); ?></button></form>';
        } else if (value.available_actions.request == true)  {
            return '<form action="<?php echo e(url('/')); ?>/account/request-asset/'+ value.id + '" method="GET"><button class="btn btn-primary btn-sm" data-toggle="tooltip" title="Request this item"><?php echo e(trans('button.request')); ?></button></form>';
        }
    ...
    function groupsFormatter(value) {
        ...
        if (value) {
            ...
            for (var index in value.rows) {
                groups += '<a href="<?php echo e(url('/')); ?>/admin/groups/' + value.rows[index].id + '" class="label label-default"> ' + value.rows[index].name + '</a> ';
            }
            return groups;
        }
    }
    ...
    function deployedLocationFormatter(row, value) {
        if ((row) && (row!=undefined)) {
            return '<a href="<?php echo e(url('/')); ?>/locations/' + row.id + '"> ' + row.name + '</a>';
        } else if (value.rtd_location) {
            return '<a href="<?php echo e(url('/')); ?>/locations/' + value.rtd_location.id + '" data-toggle="tooltip" title="Default Location"> ' + value.rtd_location.name + '</a>';
        }

    }

    function groupsAdminLinkFormatter(value, row) {
        return '<a href="<?php echo e(url('/')); ?>/admin/groups/' + row.id + '"> ' + value + '</a>';
    }

    function assetTagLinkFormatter(value, row) {
        if ((row.asset) && (row.asset.id)) {
            return '<a href="<?php echo e(url('/')); ?>/hardware/' + row.asset.id + '"> ' + row.asset.asset_tag + '</a>';
        }
        return '';
    ...
    function departmentNameLinkFormatter(value, row) {
        if ((row.assigned_user) && (row.assigned_user.department) && (row.assigned_user.department.name)) {
            return '<a href="<?php echo e(url('/')); ?>/department/' + row.assigned_user.department.id + '"> ' + row.assigned_user.department.name + '</a>';
        }
    ...
    function assetNameLinkFormatter(value, row) {
        if ((row.asset) && (row.asset.name)) {
            return '<a href="<?php echo e(url('/')); ?>/hardware/' + row.asset.id + '"> ' + row.asset.name + '</a>';
        }
    ...
    function assetCompanyFilterFormatter(value, row) {
        if (value) {
            return '<a href="<?php echo e(url('/')); ?>/hardware/?company_id=' + row.id + '"> ' + value + '</a>';
        }
    }

    function assetCompanyObjFilterFormatter(value, row) {
        if ((row) && (row.company)) {
            return '<a href="<?php echo e(url('/')); ?>/hardware/?company_id=' + row.company.id + '"> ' + row.company.name + '</a>';
        }
    }

    function usersCompanyObjFilterFormatter(value, row) {
        if (value) {
            return '<a href="<?php echo e(url('/')); ?>/users/?company_id=' + row.id + '"> ' + value + '</a>';
        } else {
            return value;
        }
    }
    ...
    function employeeNumFormatter(value, row) {
        ...
        if ((row) && (row.assigned_to) && ((row.assigned_to.employee_number))) {
            return '<a href="<?php echo e(url('/')); ?>/users/' + row.assigned_to.id + '"> ' + row.assigned_to.employee_number + '</a>';
        }
    }

    function orderNumberObjFilterFormatter(value, row) {
        if (value) {
            return '<a href="<?php echo e(url('/')); ?>/hardware/?order_number=' + row.order_number + '"> ' + row.order_number + '</a>';
        }
    }
...
<?php $__env->stopPush(); ?>
<?php /**PATH /home1/binaper1/asset.binapertiwi.co.id/resources/views/partials/bootstrap-table.blade.php ENDPATH**/ ?>